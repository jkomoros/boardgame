...
func TestMoveProgression(t *testing.T) {
	...
	singleMoveConfigs := make([]MoveProgressionGroup, len(singleMoveNames))
	multiMoveConfigs := make([]MoveProgressionGroup, len(multiMoveNames))
	...
	tests := []struct {
		...
		{
			"Optional with one partial item",
			[]string{
				singleMoveNames[0],
				singleMoveNames[2],
			},
