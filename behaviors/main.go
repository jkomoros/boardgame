...
like so: //FinishStateSetUp is called by the main engine when a State is being
created. //ConnectContainingState will have already been called, so State and
StatePropertyRef //will be set. The engine doesn't require anything to be done
in this method; it's //typically used to connect behaviors. func (g *gameState)
FinishStateSetUp() {//PlayerColor is a Connectable, which requires us to call
ConnectBehavior //on it, passing a reference to ourselvs (the struct it's
embeddded in). g.PlayerColor.ConnectBehavior(g) //If you had ohter Connectable's
in this struct, you'd call ConnectBehavior //here, too.
    }
