...
func (c *CodegenEnum) Run(p writ.Path, positional []string) {
	...
	enumOutput, err := codegen.ProcessEnums(parent.PackageDirectory)
	...
	if enumOutput != "" {
		if err := ioutil.WriteFile(filepath.Join(parent.PackageDirectory, parent.EnumOutputFile), []byte(enumOutput), 0644); err != nil {
