...
func (g *GameDelegate) ChatPolicyForPlayer(state boardgame.ImmutableState, player boardgame.PlayerIndex) boardgame.ChatPolicy {
	...
	if config.DMChatEnabled() && player >= 0 {
		userIDs := g.Manager().Storage().FetchInjectedDataForGame("", "chat.UserIDsForGame")
		// DM channels are generated by the server layer which has access to
		// user IDs. The base delegate can't generate them because it doesn't
		// have access to the user-to-seat mapping. The server layer will
		// augment this policy with DM channels.
		_ = userIDs
	}
