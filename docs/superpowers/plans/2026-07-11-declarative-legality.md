...
- [ ] Docs: `legal/doc.go` full package doc (authoring guide: the three rules of the catalog, purely-sugar guarantee, escape hatch, template tables); update `moves/doc.go` Legal section.
- [ ] **Tutorial integration (user-mandated, blocking for landing — not a token paragraph):** rework `TUTORIAL.md`'s move-legality teaching so the declarative path is taught as the PRIMARY way to express legality, with imperative `Legal()` presented as the always-available escape hatch it now is. Must include: (a) a real before/after using memory's `moveRevealCard` (the actual migrated code from Task 11, not invented snippets); (b) `WithPreconditions` + the catalog's common predicates; (c) message templates + `ConfigureLegalTemplates`; (d) `LegalCustom` for residue and `WithoutPrecondition` for opting out of inherited checks; (e) a short "what the client gets for free" section (the ledger: graying, live errors); (f) the purely-sugar framing — readers with existing games learn nothing changes for them until they opt in. Every tutorial code snippet must be compile-checked (extract to a scratch `tutorial_snippets_test.go` or verify each against the real migrated game source it's drawn from; the tutorial must not drift from code that actually exists on the branch). Also update `examples/memory/README.md`-style per-game docs if they explain Legal().
