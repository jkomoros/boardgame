...
- **The composition seam is `moves.CurrentPlayer`, `moves.Default`, `moves.DrawToPlayer`, `moves.FixUp`, `moves.FixUpMulti`, `moves.MoveComponentToSlot`, `moves.RecordCurrentPlayerChoice` and `moves.StartPhase`.** `CurrentPlayer` has a plan-aware `Legal` override and returns after its `Default.Legal` super-call evaluates a plan, avoiding duplicate proposer enforcement. `DrawToPlayer` and `MoveComponentToSlot` — the reusable "draw a card" and "play a card" verbs — declare no `Legal()` of their own at all: their legality is entirely contributed preconditions on top of `CurrentPlayer`'s, so a move embedding one keeps the full declarative surface. Other framework move types remain opaque and reject declarative configuration at boot.
...
One thing qualifying a path costs you: the automatic `stackConstraints` check. A move whose source and destination both name plain gameState stacks gets that check contributed for free, so an impossible move is rejected at `Legal()` time. The check can only read gameState, so the moment *either* end names a player-scoped stack it is silently dropped rather than contributed as an atom that would resolve to nothing. The destination's own constraints are still enforced during `Apply()`, and `MoveComponentToSlot`/`DrawToPlayer` contribute their own slot-aware check that works at every path kind — but on any other move, a player-scoped path means you should author the early check yourself with `WithLegalPreconditions`.
